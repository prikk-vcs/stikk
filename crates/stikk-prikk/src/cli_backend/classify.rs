//! Classify a non-zero prikk exit into the error taxonomy (design UD-05, RFC 007; RFC 017).
//!
...
//!   because the match was on `active wal is owned by` + `requested ref`, not on the word "lock". Every
//!   arm here follows that rule now; `PrikkError` is `#[non_exhaustive]` and its class words are
//!   prikk's to change.
...
pub fn classify(stdout: &str, stderr: &str, _category: RequestCategory) -> StikkError {
    ...
    // the lock-conflict arm below: at prikk ≤ 0.32 this message's own class word was "lock conflict:",
    // and at 0.33.0 it changed to "precondition not met:" — the match has never depended on either
    // word, only on the semantic clause ("active WAL is owned by … requested ref …"), which is exactly
    // why RFC 017 F1 found nothing broke when the class word moved.
