...
fn a_block_with_no_patch_messages_is_normal_whether_the_field_is_absent_or_empty() {
    for spelling in [r#""patch_messages": [],"#, ""] {
        ...
        assert_eq!(history.blocks[0].previous_ref_state.as_deref(), Some("c"));
    }
}
...
fn a_received_ref_is_flagged_by_its_array() {
    let text = r#"{"schema_version": "branch-list-v1",
        "branches": [{"ref_name": "heads/main", "ref_state_id": "a", "closed": true}],
        "received": [{"ref_name": "heads/theirs", "ref_state_id": "b", "closed": false}]}"#;
...
fn an_absent_received_array_is_an_empty_list() {
    let text = r#"{"schema_version": "branch-list-v1",
        "branches": [{"ref_name": "heads/main", "ref_state_id": "a", "closed": false}]}"#;
...
/// Hostile content inside a report parses to data, never to a parse failure or a panic — it is
/// rendered inert at the frontend (`C-T2a`), which is where that belongs.
#[test]
fn hostile_text_in_a_ref_name_parses_and_is_carried_through() {
    // The escape is built rather than typed, so this source file holds no raw control byte of its
    // own — and so the fixture is *valid* JSON, which requires control characters to be escaped. A
    // raw byte here would have tested the reader's tolerance of malformed input instead, which is a
    // different property and is covered in `json/tests.rs`.
    let escaped_esc = "\\u001b";
    let text = format!(
        r#"{{"schema_version": "tag-list-v1",
        "tags": [{{"ref_name": "tags/{escaped_esc}[2Jpwned", "target_block_id": "a"}}]}}"#
    );
    let tags = tags(&text).expect("hostile content is data, not a parse error");
    assert!(
        tags[0].name.chars().any(char::is_control),
        "carried verbatim to the renderer, which is what makes it inert there"
    );
}
