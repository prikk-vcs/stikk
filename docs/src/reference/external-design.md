...
| Queue | the active queue, repository-wide: depth and target ref (or prikk's missing/malformed metadata), thresholds, and each queued patch's id, operations and — at prikk ≥ 0.42 — message; `Q` or the palette; no focused ref needed. At prikk 0.39–0.41 it says once that prikk does not report a queued patch's message or content; below 0.39 it shows the depth and target and says prikk does not list queued patches. *Built by RFC 028 Handoff A (2026-09-15); was "not built" while prikk exposed no queued-patch ids (`FR-051` as amended 2026-09-06).* | UC-06, FR-051 |
...
- **TU-03 — Status bar contents** (FR-002/055; deliberately no "HEAD" — none exists; from prikk 0.42 prikk keeps a current-branch *default*, which is still not a HEAD — RFC 029): repository short name · focused ref, followed by `prikk's default: <branch>` when prikk's current branch differs from it (or is unresolved), or `no ref focused` when none is (RFC 029 Handoff B; refreshed on each Orientation read, not live; the Orientation view shows prikk's current branch whole) · queue depth (`●n queued`, hidden at 0) · worktree marker (clean/dirty/unknown) · capability badges `[RO]` `[AUT ✓/–]` `[MNT ✓/–/?]` (NFR-S01, FR-104) · background-operation indicator (`⟳ n`) · palette hint. *Amended 2026-09-06 (RFC 016 §3/§4): `[MNT]` is three-valued, not two — `✓` (adopted), `–` (no key material), `?` (key material present, adoption unverifiable on any supported prikk). `?` must never render as `✓` (`C-T2c′`).* *Amended 2026-09-15 (RFC 029 Handoff B): **the line sheds by priority when it is wider than the terminal**, each step only if it still does not fit — (1) drop the palette hint; (2) shorten the repository name from its end, marked `…`, down to one character (the header already shows it); (3) shorten prikk's default from its end, marked `…`, down to `prikk's default: …`; (4) drop that segment. The focus (or `no ref focused`), `(loading)`/`(error)`, `●n queued`, every badge and `⟳ n` are never shortened; past step 4 the line clips at the right edge. Widths are terminal cells.* *Amended 2026-09-15 (RFC 031): **a change check is not counted in `⟳ n`**, and never enters the Background Operations list; only a refresh it causes does, as `r`'s would.* Every badge has a text form (NFR-A03) and is focusable for its explanation.
...
| History (queue tier) | `Q` | Queue *(RFC 028: the tier names whose queue it is, and points here)* |
| any | seal action | Seal ceremony → History (on success) *(the Queue view has no seal action; sealing happens in the ceremony — RFC 028)* |
...
**FL-02 — Browse with filters (UC-02).** 1. History shows the queue tier above the sealed lineage (FR-010). 2. User opens filters (`f`): ref, author key id, block kind, patch purpose, path (FR-012; a message filter is now *possible* since `UD-01` retired at prikk 0.32, RFC 015, but is not built here — its own increment, RFC 015 decision 7). 3. Applying filters re-renders with an active-filters line; filters persist in the session (FR-122). 4. `/` finds ids/paths/tag text within the view (FR-013).
...
**FL-05 — Commit (UC-05).** 1. User opens Changes; stikk shows worktree-vs-baseline with per-file diffs (FR-034, via UD-03 workaround). 2. Untracked noise may be view-filtered — banner states a commit still captures those files (UD-08). 3. User invokes Commit (`C`). 4. Message prompt (required non-empty), with a notice conditioned on this session's own prikk version — not persisted below prikk 0.32, stored at or above it (`UD-01` retired, RFC 015 F2). 5. Confirmation summarizes the whole-worktree capture (file counts) and the AUTHOR key id to be used (FR-121 tier 2). 6. On yes → prikk commit runs; result (patch id, operation counts, threshold warnings) shown verbatim; Queue badge increments (FR-050/051). 7. Refusal (e.g., lock held) → TU-08.

**FL-06 — Seal ceremony (UC-06).** 1. User invokes Seal (`S`, or the palette's `op.seal`) from wherever they are focused — Orientation's queue count/target is the only preview this needs. *(Corrected 2026-09-15, RFC 028 Handoff A: this step used to say there was no dedicated Queue view yet. There is now — `Q`, listing each queued patch at prikk ≥ 0.39 — but the ceremony does not depend on it, and naming the patches in the confirmation itself is RFC 028 Handoff B.)* 2. Preview prevents an empty queue or a cross-ref target client-side (`C-T4d`) before anything is armed. 3. Confirmation (tier 3, `FR-121`) restates how many patches will seal into one block and on which ref (`FR-052`, amended 2026-09-05: prikk exposes the queued count and target ref, never the patch ids — *corrected 2026-09-15, RFC 028 Handoff B: prikk has enumerated its queue since 0.35, and at prikk ≥ 0.39 the confirmation names each patch it will freeze, by short id and, at ≥ 0.42, message, directly under the count; when the card cannot hold every row it shows as many as fit and says how many more there are and where to see them; below 0.39 it says prikk does not list queued patches*), states plainly that a trust refusal is possible, and never promises success — this is the ceremony's **first** act, an explicit yes. 4. The no-audit consent step: its own screen, unchecked, cannot be defaulted, tied to the ceremony's own irreversibility rather than to prikk's `--allow-no-audit` flag (RFC 016 §8 amended) — the **second**, distinct act. *Amended 2026-09-06 (RFC 016 decision 3): seal stays **untyped** — no MAINTAINER key id confirmation step. Two deliberate acts, not three; a third would teach users to click through the first.* *Amended 2026-09-12 (RFC 023 Handoff B): the confirmation **names the MAINTAINER key id that will sign**, as `FL-05` step 5 does for AUTHOR. This does not reopen the step above: RFC 016 removed an **act** — a key id the user had to type back — not the **information**. A ceremony that freezes patches into permanent, signed history, behind a tier-3 confirmation and a separate acknowledgement, should say which key is about to sign, and displaying it costs no extra keypress, which was decision 3's actual concern. Absent renders as nothing; there is no placeholder, because `capability_gate` refuses on `NotReady` before a confirmation is drawn.* 5. Execute → success shows the new block id and `RefState` verbatim; Orientation refreshes so the queue count reflects the seal. 6. Any refusal (a trust refusal naming what adoption requires and admitting stikk cannot verify it afterwards, RFC 016 §9; incomplete publication; a lock) → TU-08 with recovery pointers (FR-110).
...
- **OP-04 — External change & multi-writer behaviour:** on detected repository change, a passive "repository changed outside stikk — refreshed" notice; armed plans/previews are invalidated per CT-05. stikk never blocks another writer: it holds no locks between operations (NFR-R02, BD-04). *Built 2026-09-15 (RFC 031): detection is a silent change-token read — on terminal focus returning (where the terminal reports it) and every 5 seconds while idle, never while a stikk request is running or an Orientation read is pending — compared with the token stamped at the last Orientation read. On a difference, stikk re-stamps, replaces an open commit or seal confirmation (or seal's consent step) with the stale overlay, cause Repository, refreshes Orientation and the top screen in place, and shows the notice in the words above. A failed check says nothing. A worktree-only change is not in the token; RFC 030's re-read at Enter catches it.*
