...
- `env.rs` — reads `PRIKK_*` **presence** (LC-13, DM-N1). Since RFC 026 that presence answers readiness only on **prikk ≤ 0.39**; above it, readiness comes from prikk's own `key status` through `Prikk::readiness`, and `env.rs`'s second job is detecting a **stale** `PRIKK_*_SEED` that a ≥ 0.40 prikk no longer reads. It is the only module allowed to look at those variables, and it never reads a `*_SEED` **value**, only whether it is set. Threat-model anchor (A-KEY).
