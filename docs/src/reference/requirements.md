...
- **ASM-2** v1 targets prikk **≥ 0.28**, **validated through 0.32.0** (owner ruling 2026-09-04, RFC 009 decision 6; ceiling raised to 0.31.0 by RFC 012 F-e, 2026-09-05, then to 0.32.0 by RFC 015, 2026-09-06 — both only on an empirical fixture re-capture, never trusted from prikk's own changelog). **0.27.x is dropped**: its `worktree-status` is the UD-03 defect, which stikk already refuses to run, so carrying 0.27 in the range promised what stikk could not serve. Format changes before stikk v1 are absorbed by re-validation, not by stikk reading old formats itself (core refuses retired formats with migration guidance — FR-003 surfaces it). **Re-validation is a standing obligation, not a release chore:** prikk is pre-1.0 and changed its output shapes twice between 0.28 and 0.30 (RFC 009), its on-disk schema with no CLI shape change at 0.31 (RFC 012 F-e), and both its on-disk schema **and** a CLI output shape (`log` gained a per-patch line, RFC 015 F1) at 0.32 — three re-baselines in three releases, each caught only by re-running the fixtures against the real binary.
