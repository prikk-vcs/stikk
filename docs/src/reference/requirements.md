...
- **ASM-2** v1 targets prikk **≥ 0.28**, **validated through 0.33.0** (owner ruling 2026-09-04, RFC 009 decision 6; ceiling raised to 0.31.0 by RFC 012 F-e, 2026-09-05, to 0.32.0 by RFC 015, then to 0.33.0 by RFC 017, both 2026-09-06 — each only on an empirical re-verification, never trusted from prikk's own changelog). **0.27.x is dropped**: its `worktree-status` is the UD-03 defect, which stikk already refuses to run, so carrying 0.27 in the range promised what stikk could not serve. Format changes before stikk v1 are absorbed by re-validation, not by stikk reading old formats itself (core refuses retired formats with migration guidance — FR-003 surfaces it). **Re-validation is a standing obligation, not a release chore:** prikk is pre-1.0 and changed its output shapes twice between 0.28 and 0.30 (RFC 009), its on-disk schema with no CLI shape change at 0.31 (RFC 012 F-e), both its on-disk schema **and** a CLI output shape (`log` gained a per-patch line, RFC 015 F1) at 0.32, and its error-class prefixes plus a new `key`/`setup` surface at 0.33 (RFC 017) — each re-baseline caught only by re-running the fixtures, or in 0.33's case the classifier's provenance, against the real binary.
...
| **UD-09** | Per-patch **content** inspection: no `show`/`diff` on an individual patch, no `log --format json` (audit finding referenced by RFC 006/008). **Narrowed, not retired, at prikk 0.32** (RFC 015 F3): a messaged patch's **id** is now enumerable via `log`'s `patch <id>: <message>` line (RFC 015 F1) — naming a patch is no longer blocked — but its operations, preimages, and any machine-readable export remain absent | FR-030 (Patch detail as a diff, RFC 006 increment 3b — **still blocked**), FR-032/033 content diffs, FR-034's per-file diffs, FR-100/124 machine-readable exports | Block detail lists a messaged patch's id and message (RFC 015 decision 2), always beside the block's patch count so the list is never mistaken for the block's full contents. Patch detail as a rendered diff stays undelivered; do not read RFC 015 as unblocking it |
