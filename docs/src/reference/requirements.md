...
| author name/email/date | AUTHOR **key id** + signature. **No names, no emails, no dates anywhere** (no-clock design, permanent). Messages persist on prikk **≥ 0.32** and were validated-then-discarded below it (`UD-01`, **retired** — RFC 015 F2); still no author display name |
| branch | ref `heads/<name>`: a signed RefState chain. Created, listed, **closed** (never deleted) |
| HEAD / checkout a branch | **no HEAD.** Below prikk 0.42 there is no current-branch pointer; checkout materializes an explicit ref's state into the worktree. **From prikk 0.42** a `.prikk/current-branch` file names a *default* for `--ref`, moved by `prikk branch switch` — prikk calls it *"a default, never an authority"*, and it is not a HEAD. stikk keeps a client-side **focused ref** either way. From 0.42 stikk opens focused on prikk's current branch; below 0.42, or with an unresolved pointer, on `heads/main` only if it is published, and otherwise with no ref focused. Switching stikk's focus never moves prikk's (RFC 029) |
...
- **FR-003** *[M]* Refuse-and-explain unopenable targets: retired repository formats — prikk's own migration message is surfaced verbatim and classified `Environment`, grounded on the stable `no longer supports` clause (RFC 017 §5, captured live: a real format-2 repository built with prikk 0.19.0, read by 0.33.0; prikk's message says migration is **not supported** — there is no `prikk migrate`, and the classifier's prior string agreed with itself, never with prikk); missing/foreign directories (classified `Environment` — confirmed via capture that the `no such file` arm catches this, not the differently-worded arm originally written for it, which no prikk path ever emits — correct by accident until RFC 017); and version skew between stikk and the prikk core in use (ASM-2).
...
- **FR-030** *[M]* Patch detail: every operation rendered as a human-readable diff — text-span edits shown as before/after with surrounding context (the preimage travels in the patch, so no worktree is needed); creates/deletes with content summary; binary replacements as old/new blob identity + size; permission changes as old/new mode. Raw operation view (exact fields, hashes, anchors) available for P-2/P-4. *(**Unblocked at prikk 0.36** — RFC 021 F1: `prikk show <block-id|patch-id> --format json` (`show-report-v1`) renders a patch's operations, paths, **and content**, which is the surface this requirement has waited on since 0.1.0 under `UD-09`. The view itself is **not built by RFC 021** (Decision 5: its own increment). One constraint that increment inherits, from RFC 021 F2: prikk's edits are **content-anchored spans**, not line ranges — `old_span_text`/`replacement_text`, either of which may be empty. Rendering them as line diffs would be invention, and `T-T4` forbids it; the span is what prikk knows.)*
...
- **FR-033** *[M]* Compare two blocks (same or different refs): state-level difference (added / removed / content-changed / mode-changed paths), each entry expandable to content diff. This is the range-diff equivalent. *(**Amended 2026-09-12, RFC 021 Q1 as amended 2026-09-09** — this is the one requirement the 0.34–0.38 window both **widens and narrows**, because prikk answered the content question and refused the comparison one. **`prikk diff` is not coming**: prikk states comparing two arbitrary points as **a refusal, not a deferral** — a materially more expensive question they have declined to answer (RFC 021 F3). So stikk composes what it can, and says which case it is in: (a) **branch-tip comparison, in full** — `--content-path` at each tip yields two contents prikk handed over, and comparing those is not invention; (b) **same-ref range comparison, as state-level difference folded from the operations each block reports, plus each block's own spans** — honestly labelled as composed from reported operations, never presented as a byte-level diff stikk did not compute; (c) **arbitrary block-to-block comparison stays blocked**, on the new `UD-10`: `--ref` resolves a published branch only, so a block id is not addressable as a content root. The `--content-path` seam method is **not added by RFC 021** — that is Compare's own increment, which needs this design work rather than a re-baseline.)*
- **FR-034** *[M]* Worktree-vs-baseline view for the focused ref: changed / missing / untracked / unsupported paths with per-file diffs. *(→ UD-03, **resolved**; → UD-09 for the per-file diffs)* Core's `worktree-status` was broken on ordinary repositories at 0.27.x and is **verified fixed at prikk 0.28** (RFC 008), so stikk uses it directly, version-gated at ≥ 0.28 and explaining rather than running it below that. The imagined replay/plan workaround is **superseded**: it was never feasible (it needs per-file baseline content prikk does not expose, and would require stikk to read worktree bytes directly, against CON-1). The view is **path-level**; per-file content diffs waited on `UD-09`'s content half, which **retires at prikk 0.36** (RFC 021 F1 — `show --format json` carries operation content). **The view stays path-level until its own increment**: RFC 021 raised the ceiling and captured the evidence, and deliberately built no view (Decision 5). When prikk reports that the active WAL holds queued patches for a *different* ref, stikk must carry prikk's own warning verbatim — paths listed "untracked" there may be committed-but-unsealed work (RFC 009 F4). *(**prikk 0.38** additionally prints `live rename declarations: N` unconditionally, with indented `<old> -> <new>` lines under it when `N > 0`. Those are **declarations, not change entries** — stikk read them as entries and fabricated a change for a file that does not exist until RFC 021 F0 scoped the entry scan to its section.. **Re-verified at 0.41.0 (RFC 026): unchanged, and one line is new** — `refused paths: N` now sits beside `unsupported paths:`, and prikk's own source says why they are separate: *"an unrepresentable *name* and an unauthorable *entry* are different facts, and a path can be one without the other."* Individual entries carry a `[refused: <reason>]` suffix. stikk's parser is unaffected — prikk kept the entry line's first word as the change kind deliberately, "so a consumer splitting on it reads exactly what it read before" — but the count is unparsed and the suffix is shown only as part of the entry's verbatim detail **Amended 2026-09-13 (RFC 027, accepted):** the parser was *not* unaffected in one respect this note missed — prikk has always named an unrepresentable path `unsupported-path`, stikk matched `unsupported` and discarded the line, so **since 0.1.0 the Changes view has counted unsupported paths and listed none** (RFC 027 F0). At prikk ≥ 0.39 the view reads `worktree-status --format json`, marks an entry `commit` would refuse as refused with prikk's reason verbatim, and counts refusals; below 0.39 it says the verdict is unreported, never zero.)*
...
- **FR-050** *[M]* Commit: review the whole-worktree change set (FR-034), require a message input (see UD-01 for its fate), require AUTHOR signing readiness, then queue the patch. Surface prikk's own result summary (patch id, operation counts, threshold warnings) faithfully. *(**Carried 2026-09-12, RFC 021 §6 — open question for RFC 014's own follow-up, deliberately not settled here.** Verified at prikk 0.38.0: a **symlink in the worktree is reported `untracked` while `unsupported paths: 0`**, and its presence then **blocks every commit** — `error: integrity error: worktree authoring: unsupported symlink authoring: <path>: worktree symlink authoring is out of scope`. So `worktree-status` did not mark the path that commit would refuse over, and a user saw a normal-looking untracked file and an unexplained blocked commit. **Re-verified at prikk 0.41.0 (RFC 026), and prikk has answered it**: the same symlink is still `untracked` with `unsupported paths: 0`, but the report now carries `refused paths: 1` and marks the entry itself — `untracked link.txt — worktree file is not in the baseline [refused: precondition not met: link.txt: worktree symlink authoring is out of scope]`. The information stikk needed is now in the report it already reads; **what stikk does with it is not yet built** — the count is unparsed and the `[refused: …]` suffix reaches the Changes view only as trailing text inside the entry's verbatim detail. stikk's current behaviour is honest — the refusal degrades to a verbatim `Refusal`, prikk's own words, pinned by a captured test in `classify/tests.rs` — but **whether commit's preview should *prevent* this the way it prevents cross-ref and clean-worktree** (`RFC 014 decision 1`) depends on what prikk decides to do with its own report; letter 006 §4 filed it. Prevention on a shape prikk may be about to change would be stikk guessing, so this stays reported-not-prevented until prikk answers. **Decided 2026-09-13 (RFC 027, accepted, Q1 ruled (b)):** at prikk ≥ 0.39, where `worktree-status` reports `commit`'s own authoring verdict per entry, commit is **unavailable with a reason** whenever an entry is refused — the reason lists each refused path with prikk's words verbatim, and its next steps are the measured ones: remove or replace the path, or list it in `.prikkignore`, which is then part of the commit. Below 0.39 commit is offered as before and prikk's refusal is shown verbatim. `unsupported-path` entries, which prikk marks `authored` while `commit` refuses them, are not prevented on stikk's inference; prikk has been asked. **Amended 2026-09-15 (RFC 030):** a confirmed commit authors what its preview showed only as far as stikk can check. Immediately before `prikk commit`, the confirmation re-reads what prikk reports about the worktree for the previewed ref — which paths are listed, their status, prikk's verdict on each, and prikk's rename declarations — and a difference makes the confirmation stale with nothing committed; a change to prikk's current branch at ≥ 0.42 makes it stale through the change token. **The limits, stated rather than implied away:** prikk's report is path-level, so a further edit to a file already listed as modified is not detected; and a window remains between that re-check and `prikk commit` itself, in which prikk authors whatever the worktree then holds.)*
- **FR-051** *[M]* Queue review: show the queue's **depth and the ref it targets**, the active-patch warn/limit thresholds as they apply, and make "queued, not yet history" status unmistakable. *Amended 2026-09-06: this clause previously required **listing queued patches with full patch detail (FR-030)**, which is **not satisfiable** — prikk reports the active WAL only as `queued patches: N targeting <ref>` and exposes no queued-patch ids (verified at 0.32.0; RFC 013 F2 recorded the same ceiling for the seal ceremony, and RFC 015's `log` enumeration covers **sealed** patches only, never the WAL). `FR-030` itself also remains blocked on `UD-09`'s content half. The knowable half is **delivered**, distributed rather than in a dedicated view: the depth and target ref in Orientation (RFC 009 F1), the "not yet history" tier in History (RFC 006), and prikk's own threshold warning transported into the commit preview (RFC 014). A dedicated **Queue view** (`TU-01`) becomes worth building when prikk grows queued-patch enumeration — the upstream ask filed by RFC 013 — and not before, since it would list nothing.* **Re-amended 2026-09-12 (RFC 021 §5): the condition above was met — prikk 0.35 shipped `status --format json` (`status-report-v1`), which enumerates the queue** (`patches[]` with `patch_id` and per-operation `kind`/`paths`) **and carries `threshold_status`/`warn_threshold`/`hard_limit` as data** rather than leaving stikk to re-derive thresholds from environment variables. The original clause is therefore satisfiable in full, and the amendment above is kept rather than deleted because the history is the point: it records what was asked upstream, and 0.35 is the release that answered it. **The Queue view is still not built** — RFC 021 raised the ceiling and captured the evidence only (Decision 5); a capture of the awkward `unresolved_node_id` case is committed in `parse/tests.rs` for that increment to start from. The floor is 0.28, so the view must be version-conditional: below 0.35 the queue is still only a count and a target ref. **Delivered 2026-09-15 (RFC 028 Handoff A): the Queue view is built, at prikk ≥ 0.39.** It reads `status --format json` and lists each queued patch's id and operations (the kind verbatim, paths, a rename as *from → to* with its asserting key id, an unresolved node named as one), the depth, the target ref or prikk's missing/malformed metadata status, and the thresholds in stikk's words with prikk's numbers. **At ≥ 0.42 it shows each patch's message**; at 0.39–0.41 it says once that prikk does not report a queued patch's message or content. **What it does not show:** a queued patch's content, which is Patch detail's (`FR-030`) to show; and below 0.39 — including 0.35–0.38, a band stikk does not read — it shows the depth and target and says prikk does not list queued patches, never an empty list. History's queue tier now says whose queue it is.
- **FR-052** *[M]* Seal ceremony: an explicit, multi-step confirmation that (a) shows **how many** patches will seal into one block, **which ref** they target, and the block that results — *amended 2026-09-05 (RFC 013 F2): this clause previously required showing "exactly which patches", which is **not satisfiable**. prikk exposes the queued **count** and its **target ref** (`status`: `queued patches: N targeting <ref>`) and nothing that enumerates queued patch ids; `sync pending` lists **accepted** (received) patches, a different thing. The ceremony states what it knows and says plainly that it cannot enumerate them, rather than under-delivering against a requirement quietly. An enumeration surface is filed upstream, ranked below `UD-09`'s content surface* — **re-amended 2026-09-12 (RFC 021 §5): the enumeration arrived at prikk 0.35** (`status --format json` lists each queued patch's id and operations), **so "exactly which patches" is satisfiable again and the ceremony may name them.** The amendment above is kept rather than deleted: it records what the ceremony honestly said when it could not, and which release changed the answer. The ceremony as shipped by RFC 016 still says it cannot enumerate; **updating it is the seal ceremony's own follow-up, not RFC 021's**, and it must stay version-conditional — below 0.35, and down to the 0.28 floor, the count and target ref remain all prikk reports. **Delivered 2026-09-15 (RFC 028 Handoff B): at prikk ≥ 0.39 the confirmation names each patch it will freeze** — its short id, and at ≥ 0.42 its message — from the same queue read as the count, directly under the count. When the card cannot hold every row, it shows as many as fit and a remainder line saying how many more there are and where to see them (the Queue view); the consequence, its trust-refusal warning and the affordances never give up room to it. At 0.39–0.41 it says prikk does not report a queued patch's message; below 0.39 it says prikk does not list queued patches — (b) requires MAINTAINER signing readiness, (c) requires an explicit no-audit acknowledgement while core requires `--allow-no-audit`, presented as informed consent, not a pre-ticked box.
...
- **FR-055** *[S]* Focused-ref model: stikk maintains a per-repository client-side focused ref (baseline for FR-034, default target for commit/seal/checkout). Switching focus never touches the worktree by itself; the UI must never imply a HEAD moved (no such thing exists). *(**Amended 2026-09-13, RFC 029 decision 5:** true below prikk 0.42. From 0.42 prikk has a current-branch pointer — a default for `--ref`, not a HEAD and not an authority. Switching stikk's focus still never touches the worktree, and the UI still never implies a HEAD moved.)* *(**Amended 2026-09-15, RFC 029 Q1 ruled (b):** at prikk ≥ 0.42 stikk opens focused on prikk's current branch, read from `status`, and shows prikk's beside its focus as `prikk's default: <branch>` when the two differ. Below 0.42, or when prikk's pointer is unresolved, stikk opens on `heads/main` only if it is published, and otherwise with no ref focused and the ref picker open. Focus is resolved once, on the first successful read, and a later read never moves it. Switching stikk's focus never moves prikk's current branch, and never touches the worktree. Commit and seal confirmations name both refs when they differ.)*
...
- **FR-103** *[M]* Trust management: adopted maintainer keys list; add/remove with TOFU semantics surfaced — a changed key under a known id is refused by core and stikk must present this as the security event it is, not a generic error. *Amended 2026-09-06 (RFC 016 F3): the **list** half is **not satisfiable** on prikk 0.28–0.33 — `prikk trust maintainer` offers only `add`/`remove`, no way to enumerate adopted keys, and `verify`'s `sealed-block <id>: <key_id>` output is historical signer attribution, not current policy (a revoked key still prints), and does not exist before a repository's first seal — exactly when the question matters. The upstream ask is **filed, accepted and ruled** (prikk RFC 138: both `prikk trust maintainer list` and a `check --key-id` fallback, with `--format json`, and `check` exits `0` for a negative answer) but **unreleased**; stikk designs against released tags only (RFC 009's own lesson). Even once it ships, stikk's support floor (`ASM-2`, prikk ≥ 0.28) keeps the unknown state permanently reachable for every session on a version below it — this is version-conditional, not a gap awaiting closure. Until then, stikk represents adoption as a three-valued `MaintainerReadiness` (ready / not ready / **unknown**, RFC 016 §3) rather than claiming a list it cannot produce; the **add/remove/TOFU** half of this requirement is unaffected and ships as written.* **Re-amended 2026-09-12 (RFC 021 §5): prikk RFC 138 shipped in 0.34.0** — `prikk trust maintainer list` and `check --key-id` both exist, both take `--format json`, and `check` exits `0` whichever way the answer comes out (so a negative answer is data, not a failure). **The list half is satisfiable at ≥ 0.34, and `MaintainerReadiness::Ready` is constructible there for the first time.** The paragraph above stands unchanged as the record of what was true at 0.28–0.33 and why the three-valued type exists — and that type does **not** now collapse to two: stikk's floor is 0.28 (`ASM-2`), so **`Unknown` stays permanently reachable** for every session on 0.28–0.33, exactly as that paragraph predicted. This is the version-conditional case it described arriving, not the gap closing. **RFC 021 builds none of it** (Decision 5): no seam method for `trust maintainer list` is added, and the badge still reads `?` on every version until the increment that reads it lands. **Re-amended 2026-09-13 (RFC 026 B, shipped in 0.6.0): `MaintainerReadiness` is removed and replaced by `RoleReadiness`**, one per role, mirroring prikk ≥ 0.41's `key status` `binding` vocabulary — `not-adopted` is now a named, withholding state, and `Unknown` survives as the ≤ 0.39 band's answer exactly as the paragraphs above predicted.
...
- **NFR-P04** *[S]* Expensive views (block-state trees, comparisons) are computed lazily and cached per session; cache invalidation keys off repository change detection (FR-106), never off wall-clock. *(**Recorded 2026-09-12, RFC 021 §6 — prikk's own published cost curves**, carried here so a design sees them before a user does: **sealing rises quadratically with history depth**, and **checkout/merge-evidence at roughly `depth^1.45`**. Both track **depth, not repository size** — a small repository with deep history is the expensive case, which is the opposite of the intuition most tools train. **Compare over a deep range is exactly the shape that pays this** (`FR-033`, `UC-04`), so its increment must decide what it does on a deep range before it renders one: lazily-and-cached is necessary here but may not be sufficient. Not acted on by RFC 021, which builds no views.)*
...
- **ASM-2** v1 targets prikk **≥ 0.28**, **validated through 0.42.0** (owner ruling 2026-09-04, RFC 009 decision 6; ceiling raised to 0.31.0 by RFC 012 F-e, 2026-09-05, to 0.32.0 by RFC 015, then to 0.33.0 by RFC 017, both 2026-09-06, to 0.38.0 by RFC 021, 2026-09-12, to 0.41.0 by RFC 026, 2026-09-13, then to **0.42.0 by RFC 029**, 2026-09-13 — each only on an empirical re-verification, never trusted from prikk's own changelog. RFC 026's raise is the first that had to rebuild the test harness before it could measure anything: prikk 0.40 moved signing seeds out of the environment and into a key directory, so the suite's own fixture builder had stopped configuring the binary it was testing). **0.27.x is dropped**: its `worktree-status` is the UD-03 defect, which stikk already refuses to run, so carrying 0.27 in the range promised what stikk could not serve. Format changes before stikk v1 are absorbed by re-validation, not by stikk reading old formats itself (core refuses retired formats with migration guidance — FR-003 surfaces it). **Re-validation is a standing obligation, not a release chore:** prikk is pre-1.0 and changed its output shapes twice between 0.28 and 0.30 (RFC 009), its on-disk schema with no CLI shape change at 0.31 (RFC 012 F-e), both its on-disk schema **and** a CLI output shape (`log` gained a per-patch line, RFC 015 F1) at 0.32, and its error-class prefixes plus a new `key`/`setup` surface at 0.33 (RFC 017) — each re-baseline caught only by re-running the fixtures, or in 0.33's case the classifier's provenance, against the real binary. **0.34–0.38 was the fifth re-baseline and the widest** (RFC 021, five releases at once): a trust-enumeration surface at 0.34, `status --format json` and six error-class reclassifications at 0.35, `show --format json` retiring `UD-09`'s content half at 0.36, and an unconditional `live rename declarations:` section in `worktree-status` at 0.38 — the last of which made stikk report a file that does not exist until F0 fixed it. **It was also the first re-baseline run by a machine rather than performed by hand** (RFC 019's real-binary suite, `TS-07`), which is what makes "re-validation is a standing obligation" enforceable rather than aspirational.
...
|---|---|---|---|
| **UD-01** | ~~Patch messages validated then discarded~~ (audit 1A-High-2) — **RETIRED at prikk 0.32**: messages are stored (schema 4, tag 6) and `log` now enumerates a patch id and message for every patch that carries one — verified against a real prikk 0.32.0 binary (RFC 015 F1/F2). No author display name either way | FR-012 (message filter), FR-030/011 (message display), UC-02/03 richness | stikk supports prikk on both sides of the boundary: the commit-message prompt's copy is version-conditional (checked against the session's own handshake — neither a blanket "not persisted" nor a blanket "persisted" claim is true), and Block detail shows the id/message list a messaged patch carries, always beside the block's patch count so a pre-0.32 patch's absence from the list is never mistaken for its absence from history (RFC 015 F4) |
...
| **UD-09** | Per-patch **content** inspection. **Content half RETIRED at prikk 0.36** (RFC 021 F1): `prikk show <block-id\|patch-id> [--format json]` (`show-report-v1`) renders a patch's operations, paths **and content** — `edit-text` carries `old_span_text`/`replacement_text`, `create-file` carries `{"kind":"text","text":…}` plus `mode`. The id half was already narrowed at 0.32 (RFC 015 F3). **What remains is not the same gap:** (a) **no arbitrary-point comparison** — `prikk diff` is **refused, not deferred**: prikk states comparing two arbitrary points is a materially more expensive question they have declined to answer (RFC 021 F3), so this will not arrive by waiting; (b) **no `--format json` on `log`/`branch`/`tag`** — asked in letter 006 §1, and prikk offered to schedule it on our say-so, which we gave, so this one *is* pending rather than refused; (c) block-addressable content is its own dependency now — see **`UD-10`** | FR-030 (Patch detail — **unblocked at 0.36**, view not built: RFC 021 Decision 5), FR-032/033 content diffs, FR-034's per-file diffs, FR-100/124 machine-readable exports | Block detail lists a messaged patch's id and message (RFC 015 decision 2), always beside the block's patch count. **Patch detail as a rendered view is still undelivered** — the surface exists, the increment does not, and RFC 021 deliberately built none. When it is built: prikk's edits are content-anchored **spans**, not line ranges (RFC 021 F2); rendering them as line diffs would be invention |
| **UD-10** | **Block-addressable content.** `prikk show` addresses a block or patch, but the content surface (`checkout --content-path`) resolves **`--ref` only, and only a published branch**: a block id, a `tags/…` ref, and `branch create --from <block>` are all refused. Verified three ways in RFC 021. Filed upstream as letter 006 §5 | `FR-033`'s **range case** — comparing two arbitrary blocks on the same ref needs content at a block, not at a tip. (Branch-**tip** comparison is unaffected and is the case `FR-033` can serve in full) | Compare, when built, serves branch-tip comparison from `--content-path` at each tip, and the same-ref range case as state-level difference folded from each block's reported operations plus its own spans — **labelled as composed from what prikk reported**, never as a byte-level diff stikk did not compute |
