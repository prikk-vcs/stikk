...
- **T-I1 (Info-disclosure) — leaking a seed.** The catastrophic front-end failure would be a seed reaching a config file, a session file, a cache, a log, an export, a crash dump, or the screen.
  ...
  - **C-I1e** *(RFC 017 §7)* stikk never invokes `prikk key generate` or `prikk key public --seed-env` (0.33), and never offers "generate a key for me" — the first creates key material, the second reads a seed, and `C-I1a`'s presence-only discipline forbids both regardless of how the seed is subsequently displayed. This is a rule with a reason, not a style preference: prikk's own `setup` command warns that at least one seed lands in terminal scrollback, and key management is not a history browser's job. stikk points users at `prikk setup` verbatim instead of wrapping it. For the same reason, stikk never parses or quotes `setup`'s `policy: required=1` line — prikk prints it as a hard-coded literal in the voice of a query, and repeating it elsewhere would make stikk a third, independent source of a number that is not real policy.
