...
- **T-I1 (Info-disclosure) — leaking a seed.** The catastrophic front-end failure would be a seed reaching a config file, a session file, a cache, a log, an export, a crash dump, or the screen.
  ...
  - **C-I1e** *(RFC 017 §7; narrowed by RFC 026 §2, 2026-09-13)* stikk never invokes `prikk key generate` or `prikk key public` (at 0.33 spelled `--seed-env`, at 0.40+ `--seed-file`/`--role`), never invokes `prikk setup`, and never offers "generate a key for me" — the first creates key material, the second reads a seed, and `C-I1a`'s presence-only discipline forbids both regardless of how the seed is subsequently displayed. This is a rule with a reason, not a style preference: prikk's own `setup` command warns that at least one seed lands in terminal scrollback, and key management is not a history browser's job. stikk points users at `prikk setup` verbatim instead of wrapping it. **`prikk key status` is permitted, and it is the only `prikk key` subcommand that is** (RFC 026). The rule's reason was never the word `key`: `C-I1e` forbids **creating** key material and **reading a seed**, and `key status` does neither — prikk's own help calls it *"reads only, signs nothing"*, and prikk built it for exactly this consumer after stikk asked. stikk reads it to answer a question it was previously answering wrong in both directions (RFC 026 F1). The source-level guard in `cli_backend/tests.rs` was narrowed in step and **still refuses everything else**, including a subcommand prikk has not shipped yet: it fires on `key generate`, on `key public`, and on any `key <other>`, proven by probe in that increment's review. For the same reason, stikk never parses or quotes `setup`'s `policy: required=1` line — prikk prints it as a hard-coded literal in the voice of a query, and repeating it elsewhere would make stikk a third, independent source of a number that is not real policy.
...
- **T-S2 (Spoofing) — inducing use of an unsafe key.** Rendering the public example/tutorial seeds as if they were real signing keys would invite users to "sign" with a publicly-known key.
  - **C-S2** *(implemented 2026-09-13, RFC 026 §6 — previously marked NOT IMPLEMENTED here since RFC 023 F4 found it listed as in force when it never was.)* stikk recognizes the documented example public inputs and **flags them as unsafe** persistently (FR-104), by pattern of the public value — never by storing the secret. The comparison values are **public keys**, so this is literal rather than argued, and `C-I1a–e` is untouched.
    - **How it works:** prikk ≥ 0.41's `key status` reports the `public_key` that would sign; stikk compares it against `stikk_model::example_keys::PUBLISHED_EXAMPLE_KEYS` and, on a match, warns on **every** confirmation that would use it. Persistent, not a one-time notice: recomputed per confirmation from the key actually in effect, so dismissing it and carrying on does not clear it.
    - **What it covers, and what it cannot.** prikk's rule reads *"Any seed or key values published in Prikk's README, quick start, docs, tests, review packages, or issue comments … are compromised by publication."* **stikk flags the ones it can enumerate** — the keys published in prikk's user-facing documentation, which is where a person following a guide copies one from, and so is the path `T-S2` describes. A fixture added to prikk's tests tomorrow is a published example by that rule and stikk will not know. That limit is stated here rather than left for a reader to discover, because the whole point of RFC 023 F4 was a coverage claim wider than its implementation.
    - **Provenance, and the drift that remains.** Each value records its prikk file and line, read at 0.41.0, and is re-checked at each re-baseline like any captured fixture. RFC 023 Q1's plan was to *derive* these from prikk's published seeds using `key public`, so nothing would be transcribed; at 0.41 **there are no published seeds left in the user-facing docs** (`PRIKK_*_SEED` retired at 0.40, those pages rewritten), and what prikk publishes now is public keys directly — which removes the derivation but also removes that drift guard. If prikk publishes a new example key, nothing here notices; the upstream letter asking prikk to name its own published values is still the fix, and is still non-blocking.
...
- **T-R1 (Repudiation) — misattributed history.** prikk has no clocks and, below 0.32, discarded commit messages rather than persisting them (audit `UD-01`, **retired at prikk 0.32** — RFC 015 F2); a front-end could paper over either gap by fabricating times/authors, or by asserting a message's fate that this session's own prikk does not match, creating false attribution.
  - **C-R1** stikk **never fabricates** time or author name (OS-7 — permanent; prikk has no clock either way). For a message, stikk states this session's *actual*, handshake-verified status rather than a blanket claim: not persisted below prikk 0.32, stored and shown verbatim (`log`'s own `patch <id>: <message>` line, RFC 015 F1) at or above it — never asserting persistence a running prikk does not have, and never asserting discardal once it does. Refusal history and exports are stamped with the repository's canonical path + prikk version + capture time as *stikk's* record, clearly not repository authority (INV-7). *(Corrected 2026-09-05: said "repository fingerprint"; see `C-T3a` — no fingerprint exists, and `DM-02` keys by canonical path. Corrected again 2026-09-06, RFC 015: `UD-01` retired at prikk 0.32 — the message half of this control is now version-conditional, not a standing "never persisted" fact.)*
...
- **T-T3 (Tampering) — poisoned state file.** An attacker (or a corruption) alters a stikk state file to point the user at a wrong repository or replay stale filters.
  - **C-T3a** State is never authority (INV-1): every stored reference is re-resolved against prikk on load (INV-5); a focused ref prikk no longer has is discarded (LC-6); and a derived cache is used only while the **repository change token** it was computed under is still current (LC-4/LC-10). *(Corrected 2026-09-05: this control previously also claimed "a fingerprint mismatch discards stale session/cache for a moved-or-different repository". **There is no fingerprint** — RFC 003 deferred it, because prikk deliberately has no repository identity, deriving one would mean walking the entire sealed history, and it would not exist at all for a repository with no sealed blocks. The control is not weakened by its removal: `INV-5` and `LC-6` were always the clauses doing the work, which is why the fingerprint was assessed as defence in depth behind a control that already exists.)* A poisoned state file thus cannot make stikk act on a false repository fact — at worst it opens the wrong path, which the user sees.
