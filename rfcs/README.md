...
|----|-------|-----------|---------|
| 026 | [Readiness after the key directory](./accepted/026-readiness-after-the-key-directory.md) | **stikk's signing-readiness model is wrong at prikk 0.40** — the correct setup reads as not-ready; a stale export reads as ready and is refused. 0.41's `key status` answers from the computation `commit` and `seal` use. Supersedes RFC 025. **Accepted 2026-09-12, Q1 ruled (b)** | [A: re-baseline](./handoffs/026-readiness-after-the-key-directory/a-rebaseline-handoff-v1.md) · B: pending |
