...
|----|-------|-----------|---------|
| 028 | [The Queue view](./accepted/028-the-queue-view.md) | `FR-051`'s Queue view on prikk's queue enumeration, seal naming what it freezes (`FR-052`), and History's queued tier shown on refs the queue is not for. **Accepted 2026-09-13, Q1 ruled (b)** — waits for prikk's next release, which carries each queued patch's message and a queued `show` (reply 012) | pending — prikk 0.42.0 shipped it; issued after the 0.42 re-baseline (RFC 029) lands |
| 029 | [The prikk 0.42 re-baseline, and prikk's current branch](./accepted/029-prikk-0-42-rebaseline-and-the-current-branch.md) | ceiling 41 → 42 (stikk's suite already passes at 0.28 and 0.42); prikk's new current branch against stikk's client-side focused ref. **Accepted 2026-09-13; Q1 ruled (b)** | [A: re-baseline](./handoffs/029-prikk-0-42-rebaseline-and-the-current-branch/a-rebaseline-handoff-v1.md) · B: after A and RFC 030 |
| 030 | [A confirmed commit authors the worktree its preview showed](./accepted/030-a-confirmed-commit-authors-the-worktree-it-previewed.md) | RFC 029 F7: a worktree changed between commit's preview and its confirmation — a terminal `prikk branch switch` included — is committed unseen; prikk's current branch joins the change token, and commit's confirmation re-reads the worktree. **Accepted 2026-09-13** | [Confirm freshness](./handoffs/030-a-confirmed-commit-authors-the-worktree-it-previewed/confirm-freshness-handoff-v2.md) |

## Done (implemented)

| ID | Title | Shipped in | Deferred, carried forward | Handoff |
|----|-------|------------|---------------------------|---------|
| 027 | [What commit would refuse](./done/027-what-commit-would-refuse.md) | 0.7.0 candidate (on `main`) | `unsupported-path` prevention arrives with prikk's RFC 147 §3f (reply 011), verified at the next re-baseline; the would-refuse overlay's scrolling and the long-path row budget; the prose path's parse-failure attribution; an unknown ref reported against an empty baseline | [A: unsupported-path entries](./handoffs/027-what-commit-would-refuse/a-unsupported-path-entries-handoff-v1.md) · [B: verdict and prevention](./handoffs/027-what-commit-would-refuse/b-verdict-and-prevention-handoff-v1.md) |
...
| 026 | [Readiness after the key directory](./done/026-readiness-after-the-key-directory.md) | **0.6.0** | `refused paths:` — prikk now marks what `commit` would refuse; the count is unparsed and the reason is trailing text (own increment, with `FR-050`'s prevention question); `C-S2`'s residual drift over prikk's tests and issue comments | [A: re-baseline](./handoffs/026-readiness-after-the-key-directory/a-rebaseline-handoff-v1.md) · [A addendum](./handoffs/026-readiness-after-the-key-directory/a-addendum-json-validation-handoff-v1.md) · [B: readiness rebuild](./handoffs/026-readiness-after-the-key-directory/b-readiness-rebuild-handoff-v1.md) · [C: render leftovers](./handoffs/026-readiness-after-the-key-directory/c-render-leftovers-handoff-v1.md) |
| 024 | [Overlay sizing: the fix that did not travel](./done/024-overlay-sizing-and-the-fix-that-did-not-travel.md) | **0.6.0** | `render_refusal`'s lost `│ ` quoting — now a one-argument change, carried to RFC 026 B | [Overlay sizing](./handoffs/024-overlay-sizing/overlay-sizing-handoff-v1.md) |
| 023 | [Three things stikk has and does not show](./done/023-what-stikk-has-and-does-not-show.md) | **0.6.0** | `C-S2`'s implementation (with `FR-103`'s `Ready`); the example-key drift letter; the gloss reading `prikk_version`; `render_refusal`'s lost `│ ` quoting | [A: display gaps](./handoffs/023-what-stikk-has-and-does-not-show/a-display-gaps-handoff-v1.md) · [B: key-id module](./handoffs/023-what-stikk-has-and-does-not-show/b-key-id-module-handoff-v1.md) |
| 022 | [Widening the real-binary suite](./done/022-widening-the-real-binary-suite.md) | **0.6.0** | a gloss for prikk 0.28's Windows subdirectory refusal; an audit of every action's runtime; three `is_lock_conflict` clauses still source-read, each with a reachability note | [Suite widening](./handoffs/022-widening-the-real-binary-suite/suite-widening-handoff-v1.md) |
...
| 011 | [Pre-1.0 API stability policy](./done/011-pre-1-0-api-stability-policy.md) | adopted; in force from 0.2.0 | blanket `#[non_exhaustive]` — a 1.0-readiness task, with the analysis pre-done | [0.2.0 prep](./handoffs/011-pre-1-0-api-stability-policy/release-prep-handoff-v1.md) · [0.3.0 prep](./handoffs/011-pre-1-0-api-stability-policy/release-prep-0-3-0-handoff-v1.md) · [0.4.0 prep](./handoffs/011-pre-1-0-api-stability-policy/release-prep-0-4-0-handoff-v1.md) · [0.5.0 prep](./handoffs/011-pre-1-0-api-stability-policy/release-prep-0-5-0-handoff-v1.md) · [0.6.0 prep](./handoffs/011-pre-1-0-api-stability-policy/release-prep-0-6-0-handoff-v1.md) |
