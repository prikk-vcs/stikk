...
|----|-------|-----------|---------|
| 024 | [Overlay sizing: the fix that did not travel](./accepted/024-overlay-sizing-and-the-fix-that-did-not-travel.md) | **Seal's confirmation has never shown `Enter to confirm`** at any terminal height, shipped since 0.4.0; the commit prompt clips its footer; **the ref picker clips its own cursor**. RFC 016 C2 fixed this once in `render_refusal` and it did not travel — three of fourteen renderers use the fix. **Accepted 2026-09-12, Q1 ruled (c)** | [Overlay sizing](./handoffs/024-overlay-sizing/overlay-sizing-handoff-v1.md) |
