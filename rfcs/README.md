...
|----|-------|-----------|---------|
| 021 | [The prikk 0.38 re-baseline: `UD-09` retires, and a fabricated worktree entry](./accepted/021-prikk-0-38-rebaseline.md) | **Five** prikk releases at once. `prikk show` retires the content dependency carried since 0.1.0; `Ready` becomes constructible; the Queue view unblocks. **F0: on prikk ≥ 0.38 stikk fabricates a Changes entry** — wrong in shipped 0.4.1, fixed first and alone. **Accepted 2026-09-12** | [A: F0](./handoffs/021-prikk-0-38-rebaseline/f0-worktree-entry-scoping-handoff-v1.md) · [B: re-baseline](./handoffs/021-prikk-0-38-rebaseline/rebaseline-handoff-v1.md) |
| 020 | [MSRV 1.88 consistency & a supply-chain gate](./done/020-msrv-1-88-consistency-and-a-supply-chain-gate.md) | 0.5.0 candidate (on `main`) | a `schedule:` trigger for the gate (owner's, deliberately open); `[graph] all-features = false` inert until a crate declares features | [Supply chain](./handoffs/020-msrv-1-88-consistency-and-a-supply-chain-gate/supply-chain-handoff-v1.md) |
| 019 | [The real-binary integration suite](./done/019-the-real-binary-integration-suite.md) | 0.5.0 candidate (on `main`) | a `schedule:` trigger (owner's); the full `NFR-T01` platform matrix has never actually run — the Windows leg's `openssl` dependency is untested | [Integration suite](./handoffs/019-the-real-binary-integration-suite/integration-suite-handoff-v1.md) |
